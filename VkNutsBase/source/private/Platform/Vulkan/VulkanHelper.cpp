...
vk::Instance VulkanHelper::CreateInstance(const InstanceData& instanceData, uint32_t requestedExtensionsCount, const char** pRequestedExtensions) {
    ...
    if (RuntimeValidation::Enable == instanceData.validation) {
        count += 1;
        extensions = reinterpret_cast< const char** >(alloc.allocate((count + requestedExtensionsCount) * sizeof(const char*)));
    ...
    alloc.deallocate(reinterpret_cast< std::byte* >(extensions), count * sizeof(const char*));
...
vk::PhysicalDevice VulkanHelper::SelectPhysicalDevice(const vk::Instance& instance, PhsyicalDeviceSelectionStrategy strategy) {
    std::allocator< vk::PhysicalDevice > alloc {};
...
uint32_t VulkanHelper::SelectQueueFamilyIndex(const vk::PhysicalDevice& physicalDevice, const vk::SurfaceKHR& surface, vk::QueueFlags queueFlags) {
    std::allocator< vk::QueueFamilyProperties > alloc {};
    uint32_t                                    count = 0;
...
vk::SurfaceFormatKHR VulkanHelper::SelectSurfaceFormat(const vk::PhysicalDevice& physicalDevice, const vk::SurfaceKHR& surface, vk::Format preferredFormat) {
    std::allocator< vk::SurfaceFormatKHR > alloc {};
...
vk::PresentModeKHR VulkanHelper::SelectPresentMode(const vk::PhysicalDevice& physicalDevice, const vk::SurfaceKHR& surface, vk::PresentModeKHR preferredMode) {
    std::allocator< vk::PresentModeKHR > alloc {};
...
vk::Image VulkanHelper::Create2DImage(const vk::Device& device, const ImageData& imageData) {
    ...
    switch (imageData.usage) {
        case ImageUsage::MSAA: {
            imageCreateInfo.setFormat(imageData.format)
                .setExtent(imageData.extent)
                .setSamples(imageData.sampleCount)
                .setUsage(vk::ImageUsageFlagBits::eTransientAttachment | vk::ImageUsageFlagBits::eColorAttachment);
        } break;
        case ImageUsage::DepthTest: {
        ...
        case ImageUsage::ShadowMap: {
            imageCreateInfo.setFormat(vk::Format::eD32Sfloat)
                .setExtent(imageData.extent)
                .setArrayLayers(imageData.layers)
                .setSamples(imageData.sampleCount) // Should I use vk::SampleCountFlagBits::e1 instead ?
    ...
    // TODO: better manage exception
    if (vk::Result::eSuccess != device.createImage(&imageCreateInfo, nullptr, &image)) throw std::exception {};
    return image;
}

vk::Sampler VulkanHelper::CreateSampler(const vk::Device& device, const SamplerData& samplerData) {

    vk::SamplerCreateInfo samplerCreateInfo {};

    samplerCreateInfo.setMagFilter(samplerData.magFilter)
        .setMinFilter(samplerData.minFilter)
        .setMipmapMode(samplerData.mipmapMode)
        .setAddressModeU(samplerData.addressMode)
        .setAddressModeV(samplerData.addressMode)
        .setAddressModeW(samplerData.addressMode)
        .setMipLodBias(samplerData.mipLodBias)
        .setAnisotropyEnable(samplerData.anistropyEnable)
        .setMaxAnisotropy(samplerData.maxAnistropy)
        .setCompareEnable(samplerData.CompareEnable)
        .setCompareOp(samplerData.compareOp)
        .setMinLod(samplerData.minLod)
        .setMaxLod(samplerData.maxLod)
        .setBorderColor(samplerData.borderColor)
        .setUnnormalizedCoordinates(samplerData.unnormalizedCoords);

    vk::Sampler sampler;
    // TODO: Better manage exception
    if (vk::Result::eSuccess != device.createSampler(&samplerCreateInfo, nullptr, &sampler)) throw std::exception {};
    return sampler;
}
